...
// 註冊遊戲商會員
func createUserAPI(user CreateUserReq, url string) (CreateUserRes, error) {
	resp := CreateUserRes{}
...
func GameAccountExist(username string) (*CheckUserRes, error) {
	...
	resp, err := checkUserAPI(
		...
		Httpdomain+string(CheckUserUri),
	)
	if err != nil {
		mlog.Error(err.Error())
		return nil, err
...
// 查詢真人打賞注單
func rewardListGetAPI(user RewardListGetApiReq, url string) (RewardListGetApiRes, error) {
	resp := RewardListGetApiRes{}
...
// 取得真人超級彩金
func getSuperLotteryAPI(url string) (GetSuperLotteryRes, error) {
	resp := GetSuperLotteryRes{}
	...
	// Get method
	err := apicaller.WgGameSendGetRequest(url, header, handler)
	if err != nil {
		log.Error(err.Error())
		return resp, err
	}
...
// 真人超級彩金(小彩金)發放紀錄
func superLotteryListAPI(user SuperLotteryListReq, url string) (SuperLotteryListRes, error) {
	resp := SuperLotteryListRes{}
...
func AccountRegister(tx *sql.Tx, mid int, member MemberGameAccount) error {
	...
	resp, err := createUserAPI(
		...
		Httpdomain+string(CreateUserUri),
	)
	if err != nil {
		mlog.Error(err.Error())
		tx.Rollback()
...
// 	err := row.Scan(&gameAccount.UserName, &gameAccount.NickName)
// 	if err != nil {
...
func AccountLogin(login ForwardGameReq) (string, error) {
	resp, err := getURLTokenAPI(
		login,
		Httpdomain+string(LiveForwardGameUri),
	)
	if err != nil {
		mlog.Error(err.Error())
		return "", err
